...
### Conservative tile rejection experiment

Select `?bitmaskVariant=reject` (append `&geometry=full` to bypass LOD). Original, cached and triangle-owned variants remain available unchanged as comparisons. The new variant builds on triangle-owned masks:

- Before allocating a bin entry, test each clipped triangle fan against the tile's pixel-centre rectangle with edge equations. Reject only when one edge excludes the entire rectangle; both fans must fail for a clipped quadrilateral. A scale-dependent floating-point margin retains uncertain boundaries. Partial viewport tiles use their actual pixel centres.
- Bin into eight distance buckets per tile, visiting nearby buckets first. This is coarse ordering by source triangle centroid distance in 16-metre intervals, not an exact depth sort; order within buckets is unspecified. Ordering affects efficiency, not the lowest-ID equal-depth winner rule.
- Each pixel publishes its nearest depth after a batch. The following batch computes the maximum of those depths for the whole tile. Uncovered pixels remain at 1 and prevent rejection. Pixels outside a partial viewport tile do not block rejection. Reject a triangle/tile candidate only if its minimum clipped vertex depth, reduced by an area-conditioned uncertainty margin, is strictly farther than that maximum. Thin/degenerate fans weaken or disable rejection. This conservative whole-tile rule deliberately misses some opportunities around foliage holes.
- Rejected candidates skip fine coverage and mask construction. Depth comparison still happens after transformation/clipping, and repeated setup per tile is not eliminated in this pass. Pool-overflow tiles retain exhaustive software scanning (without bucket ordering), applying the same conservative depth rule as they scan.

Statistics are sampled on the existing asynchronous readback frames: bounding-box candidate tiles, edge-rejected bin candidates, depth-rejected raster candidates / raster candidates, coverage sample tests, mask-selected depth tests and winning depth/ID updates. Winner updates count intermediate improvements, not unique final visible pixels; the covered-pixel counter remains separate. Extra counters are 32-bit and can wrap in pathological workloads; diagnostic atomics can perturb sampled-frame performance. Edge and depth rejection counts have different denominators, especially during overflow scans.

Tile heads now use 36 bytes/tile (eight heads plus overflow flag) instead of 8. The per-tile depth scratch adds 256 bytes plus small scalars, keeping workgroup storage around 4.2 KiB rather than bringing back the 8 KiB sample-depth cache. The entry pool and geometry/resolution stay unchanged.

CPU tests cover random edge-rejection candidates, partial tiles, clipped quadrilaterals, holes, ties, depth variation, skinny triangles and multi-batch bucket ordering against exhaustive resolution. Naga parses/emits the WGSL. These are not a device-level floating-point proof or GPU performance validation; inspect silhouettes and foliage during phone comparison. No speedup is claimed until measured.
