...
**Patch LOD** selects among independent LOD chains for spatial patches. **Full resolution** bypasses all meshlet compute. **Hierarchical LOD** uses `buildHierarchyAsset.js` and a distinct GPU traversal generated by the renderer. The remaining group-LOD description below applies to Patch LOD.
...
Patch LOD uses a bounded per-group cluster loop. Hierarchical LOD uses stackless per-instance tree traversal with a cluster loop for each accepted node. A future parallel work queue could distribute large individual assets across more GPU invocations.
...
The geometry scene and ordinary Three.js occluders render into an HDR render target with a float depth texture.
