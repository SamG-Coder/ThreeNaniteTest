...
4. The deployment builds all three valley densities and the shared tree ahead of time. Browsers download typed hierarchy metadata and 32-page gzip batches, retaining eight recent batches. Root pages permit startup before detailed pages arrive. Custom/unprepared assets use the worker/IndexedDB fallback below.

   Variable-sized geometry pages are compressed with gzip into IndexedDB, keyed by the source attributes and a format version. Only root and pending page data need to remain in the CPU page cache. Quota/storage failures retain an in-memory fallback. Source geometry and hierarchy metadata remain in memory for the original renderer and traversal. The fallback is local persistent streaming; prepared valley assets stream over HTTP.
...
First-time cooking is substantial: the full source tree took about 142 seconds in the reference CPU environment and produced roughly 66 MB of raw asset data before page compression. The deployed valley now does this work in GitHub Actions, with cached outputs reused across unrelated code changes. The worker fallback is for unprepared assets. No mobile speedup is claimed until measured on the same camera, viewport and quality settings.

## Loading and idle-work fixes

Camera movement, projection, viewport, quality and page-residency changes invalidate selection. Unchanged geometry visibility and shading are reused while Three.js continues to render the animated water. Output-mode changes and recreated targets invalidate that visibility cache. The HUD labels cached static geometry explicitly; stationary FPS is not moving-camera performance. GPU regression tests assert both reuse and invalidation on rotation.

The page scheduler sorts priorities once per demand update, rather than repeatedly for each uploaded replacement. Residency updates patch only changed 4-byte child counts; they no longer copy and upload the entire hierarchy table each streaming frame. Look gestures now clear stale pointer state on blur/disable and support mouse dragging without pointer lock. Mobile status text lets swipes reach the canvas.
