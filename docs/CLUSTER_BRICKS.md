...
The page scheduler sorts priorities once per demand update, rather than repeatedly for each uploaded replacement. Look gestures now clear stale pointer state on blur/disable and support mouse dragging without pointer lock. Mobile status text lets swipes reach the canvas.
