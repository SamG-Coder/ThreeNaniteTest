...
export const forestRejectWGSL = /* wgsl */`
...
// Conservative edge/rectangle test. A rejected fan cannot cover any pixel
// centre in the tile. Margin deliberately admits uncertain float boundaries.
fn outsideEdge(a:vec2<f32>,b:vec2<f32>,lo:vec2<f32>,hi:vec2<f32>)->bool {
  let e=max(max(edge(a,b,lo),edge(a,b,hi)),max(edge(a,b,vec2<f32>(lo.x,hi.y)),edge(a,b,vec2<f32>(hi.x,lo.y))));
  let delta=abs(b-a);let extent=max(abs(lo-a),abs(hi-a));
  let margin=1e-4+1e-5*(delta.x*extent.y+delta.y*extent.x);
  return e < -margin;
}
fn fanMayCover(a:vec4<f32>,b:vec4<f32>,c:vec4<f32>,lo:vec2<f32>,hi:vec2<f32>)->bool {
  return !outsideEdge(c.xy,b.xy,lo,hi)&&!outsideEdge(b.xy,a.xy,lo,hi)&&!outsideEdge(a.xy,c.xy,lo,hi);
}
fn tileMayCover(t:Triangle,tile:vec2<u32>)->bool {
  let lo=vec2<f32>(tile*8u)+vec2<f32>(.5);
  let hi=vec2<f32>(min(tile*8u+vec2<u32>(7u),params.size.xy-vec2<u32>(1u)))+vec2<f32>(.5);
  return fanMayCover(t.a,t.b,t.c,lo,hi)||(t.info.y==4u&&fanMayCover(t.a,t.c,t.d,lo,hi));
}
fn conservativeNearest(t:Triangle)->f32 {
  ...
  var area=abs(edge(t.a.xy,t.b.xy,t.c.xy));
  if(t.info.y==4u){area=min(area,abs(edge(t.a.xy,t.c.xy,t.d.xy)));}
  // Skinny/ill-conditioned triangles get a wider uncertainty interval. A
  // degenerate clipped fan disables depth rejection, not rasterization.
  if(area<=1e-8){return -1e30;}
  let maxDepth=max(1.0,max(max(abs(t.a.z),abs(t.b.z)),max(abs(t.c.z),abs(t.d.z))));
  let slack=1e-5*max(1.0,span*span/area)*maxDepth;
  return min(min(t.a.z,t.b.z),min(t.c.z,t.d.z))-slack;
}
...
fn clear(@builtin(global_invocation_id) gid:vec3<u32>) {
  if(gid.x<params.size.w) {for(var b=0u;b<8u;b++){atomicStore(&heads[gid.x*9u+b],EMPTY);}atomicStore(&heads[gid.x*9u+8u],0u);}
  if(gid.x>=2u&&gid.x<16u) {atomicStore(&control[gid.x],0u);}
}
@compute @workgroup_size(64)
fn bin(@builtin(global_invocation_id) gid:vec3<u32>) {
  ...
  let bucket=min(7u,u32(max(0.0,t.normal.w)/16.0));
  var candidates=0u;var rejected=0u;
  for(var y=first.y;y<=last.y;y++){for(var x=first.x;x<=last.x;x++){
    candidates++;
    if(!tileMayCover(t,vec2<u32>(x,y))){rejected++;continue;}
    ...
    if(entry<params.settings.x){
      let previous=atomicExchange(&heads[tile*9u+bucket],entry);
      entries[entry]=vec2<u32>(id,previous);
    }else{
      if(atomicExchange(&heads[tile*9u+8u],1u)==0u){atomicAdd(&control[3],1u);}
    }
  }}
  if(params.settings.z!=0u){atomicAdd(&control[6],candidates);atomicAdd(&control[7],rejected);}
}
...
var<workgroup> pixelDepth:array<f32,64>;
var<workgroup> tileFarthest:f32;
var<workgroup> currentBucket:u32;
...
fn raster(@builtin(workgroup_id) group:vec3<u32>,@builtin(local_invocation_id) local:vec3<u32>){
  ...
  pixelDepth[local.x]=select(0.0,1.0,coord.x<params.size.x&&coord.y<params.size.y);
  if(local.x==0u){currentBucket=0u;nextEntry=atomicLoad(&heads[tile*9u]);scanCursor=0u;}
  workgroupBarrier();
  loop{
    if(local.x==0u){
      tileFarthest=0.0;
      for(var pixel=0u;pixel<64u;pixel++){tileFarthest=max(tileFarthest,pixelDepth[pixel]);}
      batchCount=0u;
      if(atomicLoad(&heads[tile*9u+8u])!=0u){
        ...
        // Coarse front-to-back order establishes depth before distant buckets.
        while(nextEntry==EMPTY&&currentBucket<7u){currentBucket++;nextEntry=atomicLoad(&heads[tile*9u+currentBucket]);}
    ...
    if(local.x<count){
      ...
      let nearest=conservativeNearest(t);
      // Uncovered pixels retain depth 1, preventing whole-tile rejection.
      // Strict separation with slack retains ties and floating-point uncertainty.
      let hidden=tileFarthest<1.0 && nearest>tileFarthest+1e-5;
      if(params.settings.z!=0u&&t.info.y>=3u){atomicAdd(&control[12],1u);if(hidden){atomicAdd(&control[8],1u);}}
      var samples=0u;
      if(t.info.y>=3u&&!hidden){
      ...
      if(params.settings.z!=0u){atomicAdd(&control[9],samples);}
    ...
    var resolves=0u;var wins=0u;
    while(bits!=0u){
      resolves++;
      let bit=firstTrailingBit(bits);let t=batchTriangles[bit];let z=sampleDepth(t,point);
      if(z>=0.0 && (z<best || (z==best && t.info.x<winner))){best=z;winner=t.info.x;color=shade(t);wins++;}
      bits=bits&(bits-1u);
    }
    if(params.settings.z!=0u){atomicAdd(&control[10],wins);atomicAdd(&control[11],resolves);}
    pixelDepth[local.x]=select(0.0,best,coord.x<params.size.x&&coord.y<params.size.y);
    workgroupBarrier();
  }
