...
export class DemoUI {
  ...
  updateStats(stats) {
    ...
    if(stats.bitmask?.lighting){const l=stats.bitmask.lighting;if(l.shadows||l.reflections)this.elements.geometryReadout.textContent+=` · ray ${l.shadows?'shadows ':''}${l.reflections?'reflections ':''}· ${compactFormatter.format(l.rays)} rays / ${compactFormatter.format(l.limited)} budget fallbacks`;}
    if(stats.bitmask?.reused)this.elements.geometryReadout.textContent+=' · cached static geometry';
