...
export class GeometryCache{
 ...
 evict(){
  ...
  if(!candidates.length)return false;const id=candidates[0];this.resident.delete(id);this.publish();
 ...
 prefetch(unit){if(!this.asset.pageProvider)return;
  for(const id of unit.pages){if(this.mapping.has(id)||this.asset.pages[id]||this.loading.has(id))continue;
   const promise=this.asset.pageProvider.get(id).then(page=>{if(this.pending?.unit.id===unit.id)this.asset.pages[id]=page;}).catch(error=>{this.pageError=error;console.warn('Geometry page load failed; retaining parent',error);}).finally(()=>this.loading.delete(id));this.loading.set(id,promise);
  }
 }
 ...
 metadata(){const a=this.asset.groupLods.slice();for(const unit of this.layout.units)if(!this.resident.has(unit.id))a[unit.id*24+5]=0;return a;}
}
