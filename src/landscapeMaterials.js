...
import {Fn,attribute,If,float,texture,int} from 'three/tsl';
...
export function createLandscapeTexture(){
 ...
 for(let layer=0;layer<4;layer++)for(let y=0;y<size;y++)for(let x=0;x<size;x++){
  ...
  const i=(layer*size*size+y*size+x)*4,b=Math.round(Math.min(1,value)*255);data.set([b,b,b,b],i);
 }
 const result=new THREE.DataArrayTexture(data,size,size,4);result.generateMipmaps=true;result.minFilter=THREE.LinearMipmapLinearFilter;result.magFilter=THREE.LinearFilter;result.wrapS=result.wrapT=THREE.RepeatWrapping;result.needsUpdate=true;result.name='Willowmere soil, mineral, bark and leaf textures';return result;
}
export function createLandscapeMaterial(map){
 const mat=new THREE.MeshStandardNodeMaterial({vertexColors:false,roughness:.87});
 mat.colorNode=Fn(()=>{
  const surface=attribute('surface','float'),uv=attribute('uv','vec2');
  const detail=float(1).toVar();If(surface.lessThan(3.5),()=>{detail.assign(texture(map,uv).depth(int(surface)).r.mul(1.5));});
