...
test('forest WGSL parses and emits including workgroup mask synchronization',()=>{
  ...
  try { for(const code of [visibilityWGSL,cullWGSL,pyramidWGSL,pyramidBaseWGSL,forestFastWGSL,forestBoundedWGSL,forestDispatchWGSL,forestRasterWGSL,forestReferenceWGSL,forestOwnedMaskWGSL,forestRejectWGSL]) { const module=frontend.parse(code);try{assert.ok(module.to_wgsl().length>0);}finally{module.free();} } }
